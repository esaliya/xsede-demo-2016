...
MDS_OPS="-DTimingFile=$TimingFile -DAlpha=$Alpha -DThreshold=$StressThreshold -DStressIterations=$StressIterations -DCGErrorThreshold=$CGErrorThreshold"
...
$BUILD/bin/mpirun --hostfile $7 --report-bindings --map-by ppr:$ppn:node:SPAN  --bind-to $bindToUnit --rank-by core  -np $(($nodes*$ppn)) java $opts  $MDS_OPS -cp $cp edu.indiana.soic.spidal.damds.Program -c config.properties -n $nodes -t $tpp -mmaps $mmaps -mmapdir $mmapdir 2>&1 | tee $pat.$xmx.$memmultype.$4.$3.comm.$commpat.out.txt
